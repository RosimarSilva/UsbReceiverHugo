...
/*Esta macro coloca em nivel lógico high ou low o pino
 * que aciona um transitor para gerar uma tensao pino 54 PTC5  ####VERIFICAR DIREITO ISSO*/
#define Tensao2_PutVal(output)  \
FGPIO_PinWrite(GPIOC, 5,output); \
GPIOC->PDDR |= (1U << 5U)

/*Esta macro coloca em nivel lógico high ou low o pino
 * que aciona um transitor para gerar uma tensao pino 53 PTC4 ####VERIFICAR DIREITO ISSO*/
#define Tensao1_PutVal(output)  \
FGPIO_PinWrite(GPIOC, 4,output); \
GPIOC->PDDR |= (1U << 4U)
...
void setRtc(uint8_t day,uint8_t month,uint16_t year,uint8_t hour,uint8_t minute);//configuradia e hora
...
void tela1(void);
void screen(void);
void sinalFunc(bool enanble);// envia para o programa se o siatema está dormindo ou acordao
